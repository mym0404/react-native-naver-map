...
<Tabs items={['Camera', 'Region']}>
  <Tabs.Tab>
    **Camera**: 정확한 줌 레벨과 각도 제어
    ```tsx filename="CameraExample.tsx"
  ...
  <Tabs.Tab>
    **Region**: 표시할 영역 범위로 설정
    ```tsx filename="RegionExample.tsx"
...
```tsx filename="CameraControlExample.tsx"
...
```tsx filename="AnimateCameraTo.tsx"
// 기본 사용법
mapRef.current?.animateCameraTo({
  latitude: 35.1796,
  longitude: 129.0756,
...
```tsx filename="AnimateCameraBy.tsx"
// 화면 픽셀 단위로 상대 이동
...
```tsx filename="AnimateRegionTo.tsx"
// 특정 영역을 화면에 맞춰 표시
...
```tsx filename="TwoCoordsAnimation.tsx"
...
```tsx filename="CancelAnimation.tsx"
...
```tsx filename="MoveToCurrentLocation.tsx"
...
```tsx filename="CoordinateConversion.tsx"
// 화면 좌표를 지도 좌표로 변환
...
```tsx filename="DebouncedCameraChange.tsx"
